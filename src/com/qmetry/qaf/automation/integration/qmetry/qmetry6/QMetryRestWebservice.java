...
public class QMetryRestWebservice {
	...
	QMetryRestWebservice(String baseURL, String user, String password) {
		...
		log.info("Qmetry6 scheduled prj: " + prj + " rel : " + rel + " build: " + build + " platform: " + platform
				+ " drop: " + drop + "cycle:" + cycle);
	...
	public String login() {
		...
		try {
			response = new RestTestBase().getWebResource(serviceUrl, "/rest/login").getRequestBuilder()
					.header("Accept", "application/json").header("Content-Type", "application/x-www-form-urlencoded")
					.post(String.class, authentication1);

			JsonObject jsonRequest = new Gson().fromJson(response, JsonElement.class).getAsJsonObject();
	 ...
	 */
	public boolean executeTestCase(String testSuiteID, String testSuiteRunID, String testCaseID, String status) {
		...
		if (testSuiteRunID == null || StringUtil.isBlank(testSuiteRunID)) {
			...
			JsonObject jsonTestSuite = new Gson().fromJson(suiteResponse, JsonElement.class).getAsJsonObject();
		...
		JsonObject jsonTcs = new Gson().fromJson(response, JsonElement.class).getAsJsonObject();
	 ...
	 */
	public boolean executeTestCaseUsingRunId(String testSuiteID, String testSuiteRunID, String tcRunID,
			String qmetryResult) {
		log.info(String.format(
		...
		if (testSuiteRunID == null || StringUtil.isBlank(testSuiteRunID)) {
			...
			JsonObject jsonTestSuite = new Gson().fromJson(response, JsonElement.class).getAsJsonObject();
			JsonArray arrTestSuiteRunID = jsonTestSuite.get("data").getAsJsonArray();
			for (JsonElement jsonElement : arrTestSuiteRunID) {
				if (jsonElement.getAsJsonObject().get("platformID").getAsString().equalsIgnoreCase(platform)) {
					testSuiteRunID = jsonElement.getAsJsonObject().get("tsRunID").getAsString();
				}
			}
	...
	public String getTestCaseRunID(String testSuiteRunId) {
		...
		Builder builder = new RestTestBase().getWebResource(serviceUrl, "/rest/execution/list/tcr").getRequestBuilder();
		// add header
	...
	public String getTestSuiteRunID(String testSuiteID) {
		...
		Builder builder = new RestTestBase().getWebResource(serviceUrl, "/rest/execution/list").getRequestBuilder();
		// add header
		builder.header("Accept-Charset", "utf-8");
		for (Iterator<Map.Entry<String, String>> it = getRequestHeader().entrySet().iterator(); it.hasNext();) {
			Map.Entry<String, String> entry = it.next();
	...
	public String getUserInfo() {
		Builder builder = new RestTestBase().getWebResource(serviceUrl, "/rest/admin/project/getinfo")
				.getRequestBuilder();
	...
	public String updateStatus(String testCaseRunId, String testSuiteRunID, TestCaseRunResult runResult) {
		...
		Builder builder = new RestTestBase().getWebResource(serviceUrl, "/rest/execution/runstatus/bulkupdate")
				.getRequestBuilder();
	...
	public String createTestSuiteFolder(String parentFolderId) {
		...
		Builder testCaseFolderBuilder = new RestTestBase().getWebResource(serviceUrl, "/rest/testsuites/folders")
				.getRequestBuilder();
	...
	public String searchExistSuiteUsingTCID(String testCaseId) {
		Builder builder = new RestTestBase().getWebResource(serviceUrl, "/rest/testsuites/list").getRequestBuilder();
		// add headers
		for (Iterator<Map.Entry<String, String>> it = getRequestHeader().entrySet().iterator(); it.hasNext();) {
			Map.Entry<String, String> entry = it.next();
			builder.header(entry.getKey(), entry.getValue());
	...
	public String createTestCase(String scriptName, String folderId) {
		...
		Builder tcBuidler = new RestTestBase().getWebResource(serviceUrl, "/rest/testcases/").getRequestBuilder();
		...
		JsonObject userInfoJson = new Gson().fromJson(userInfo, JsonElement.class).getAsJsonObject();
	...
	public String createTestSuite(String testSuiteFolderId) {
		...
		Builder tsBuilder = new RestTestBase().getWebResource(serviceUrl, "/rest/testsuites/").getRequestBuilder();
	...
	public String linkPlatform(String newTestSuiteId) {
		...
		Builder linkPlatformBuilder = new RestTestBase().getWebResource(serviceUrl, "/rest/testsuites/link/platforms")
				.getRequestBuilder();
		...
		String response = null;
		try {
			response = linkPlatformBuilder.put(String.class, linkPlatformRequest.toString());
		} catch (Exception e) {
			e.printStackTrace();
		}
		return response;
	...
	public String linkTestCaseWithSuite(String testSuiteKey, String testCaseKey) {
		...
		Builder testCaseBuilder = new RestTestBase().getWebResource(serviceUrl, "/rest/testcases/link/testsuites")
				.getRequestBuilder();
	...
	public String executeTestCaseToGetTestSuiteId(String testCaseId) {
		...
		Builder testCaseBuilder = new RestTestBase().getWebResource(serviceUrl, "rest/testcases/execute")
				.getRequestBuilder();
		// add headers
		for (Iterator<Map.Entry<String, String>> it = getRequestHeader().entrySet().iterator(); it.hasNext();) {
			Map.Entry<String, String> entry = it.next();
	...
	public String createTestCaseFolder(String parentFolderId) {
		...
		Builder testCaseFolderBuilder = new RestTestBase().getWebResource(serviceUrl, "/rest/testcases/folders")
				.getRequestBuilder();
